...
- New project_hash + project_name injection in the relay's outbound request path. Every MCP tools/call body that targets `drift_declare_intent`, `drift_broadcast_change`, or `drift_claim_files` now gets project_hash + project_name attached from a hook-managed breadcrumb at `~/.drift/state/current-project.json` when the caller didn't supply them in arguments. Closes the bug class where a tool call with bare filenames (no slashes) collapsed to the wrong project_id on the server because file-tree-based attribution can't match a relative basename against the server's stored absolute paths. The hook writes the breadcrumb every time `drift-check` fires (so the file always reflects the most recent project the developer interacted with via Claude Code), the relay reads it on each outbound request and injects fields into `params.arguments` before the encryption pipeline runs. The injected fields live alongside the existing `files` / `lock_type` arguments and pass through encryption as plaintext metadata (same treatment as `file_path` and `developer_id`). When no breadcrumb exists, when the body isn't a JSON-RPC tools/call envelope, when the tool isn't project-aware, or when arguments already carry a project_hash, the original body is forwarded byte-for-byte unchanged. Pairs with the new server-side `projects.findOrCreateByHash` repository method + the `projects.project_hash` column added in DB migration 078.
- New `internal/config/currentproject.go` with `WriteCurrentProjectState` / `ReadCurrentProjectState`. State file is mode 0600, atomic-write via the existing `AtomicWriteFile` helper. Schema is versioned (currently v1) so future fields can land without breaking older relays.
- Linux Secret Service fallback. When `keyring.Set` / `keyring.Get` / `keyring.Delete` return a "no Secret Service" / "no session bus" / "org.freedesktop.secrets not activatable" error (the standard signature of a headless Linux box without gnome-keyring, an SSH-only server, a Docker container without dbus, or a CI runner), the keychain package now routes the call to an encrypted `~/.drift/.secrets` file instead. AES-256-GCM with a fresh random nonce per write. The AES key is HKDF-SHA256-derived from `/etc/machine-id` (or `/var/lib/dbus/machine-id` on systemd-less distros) with a fixed `drift-cli/keychain-fallback/v1` info string for domain separation. The file is mode 0600 and bound to the host: copying it to another machine yields a decrypt failure because the derived key changes with `machine-id`. macOS + Windows: zero behavior change, keyring is always available. Emits one stderr line + one structured `[keychain] fallback_engaged` log entry on first activation per process so customers can grep for it from `drift status` output later.

### Fixed

- v0.1.0 through v0.1.23 on a headless Linux server (or in a fresh Docker container) would error out of `drift install` with `keychain set token: failed to unlock correct collection ...` and refuse to start the relay. Customers had to manually install gnome-keyring + start a D-Bus session before re-running install, which is impossible inside most CI / container / serverless contexts. v0.1.24 falls through to the encrypted file backend automatically.
- MCP tool calls that referenced files by bare filename (e.g. `drift_claim_files` with `["foo.go"]` instead of `["src/foo.go"]`) previously had a chance of attributing to the wrong project on the server when two different projects in the same org happened to contain a file with that basename. The server's path-based fallback collapsed those onto whatever project's row matched first. v0.1.24 always sends a project_hash from the hook breadcrumb when one exists, so the server resolves the project deterministically and the path-fallback only runs for genuinely first-time projects with no breadcrumb yet.
...
- New `promptForTokenInteractive(stdout)` uses the same `huh` TUI library `drift quickstart` already uses, so the prompt looks consistent with the rest of the install flow. Title bar reads "Drift API token required", description points the customer at `https://app.driftlabs.io/profile`, input field uses `EchoModePassword` so the token isn't echoed in plain text in the customer's terminal scrollback. Inline validator runs `config.ValidateToken` so a malformed paste gets rejected immediately instead of erroring later in the install.
- Falls back to the existing deferred-token note when huh can't render (no TTY: CI, scripted contexts, customers piping install output through tee, etc.) or when the customer presses enter without pasting a value. Same exit code behavior as before in those cases, so any automation that relied on the silent-skip behavior still works.
- New `printDeferredTokenNote(stderr)` extracted from the old inline message so both the prompt-failed and customer-skipped paths share one source of truth for the fallback wording.
...
- `RegisterClaudeCodeHooks` and `RegisterClaudeCodeHooksGlobal` now run the executable path through new `normalizeExePathForHook` before composing the hook command string. Replaces every backslash with a forward slash. The Win32 API accepts both forms (cmd.exe, PowerShell, and bash on Windows all resolve `C:/Users/tony/...` correctly), so this is a safe rewrite that fixes bash invocation without forcing customers to add a `shell: "powershell"` field to the hook entry.
- New `TestNormalizeExePathForHook` pins four cases: Windows backslash path → forward slash, Windows path with spaces → forward slash with spaces preserved, already-normalized forward slash path → no change, Unix path → no change. Test added to `internal/clients/claudecode_hook_test.go`.
